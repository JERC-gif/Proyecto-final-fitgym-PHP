...
@section('title', 'Crear Membresía - FitGym')
@section('page_title', 'Crear Nueva Membresía')
...
@section('content')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="tipo_membresia" name="tipo_membresia"
                        ...
                        <option value="Visita" {{ old('tipo_membresia') == 'Visita' ? 'selected' : '' }}>Visita</option>
                        <option value="Semana" {{ old('tipo_membresia') == 'Semana' ? 'selected' : '' }}>Semana</option>
                        <option value="Mes" {{ old('tipo_membresia') == 'Mes' ? 'selected' : '' }}>Mes</option>
                    </select>
                    <input type="hidden" id="nombre" name="nombre" value="{{ old('nombre') }}" required>
                    @error('nombre')
                ...
                <div>
                    ...
                    <input type="number" id="duracion_dias" name="duracion_dias" value="{{ old('duracion_dias') }}" min="1"
                           class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2.5"
                ...
                <div class="flex items-center pt-8">
                    <input id="activa" name="activa" type="checkbox" value="1" {{ old('activa', true) ? 'checked' : '' }}
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="descripcion" name="descripcion" rows="3"
                              class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2.5"
                              placeholder="Descripción de la membresía...">{{ old('descripcion') }}</textarea>
                    @error('descripcion')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Si hay un valor old, actualizar al cargar
            if (tipoSelect.value) {
